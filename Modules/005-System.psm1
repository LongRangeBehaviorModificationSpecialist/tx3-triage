...
function Get-MappedNetworkDriveMRU {
    ...
    try {
        ...
        $ExecutionTime = Measure-Command {
            Show-Message("$Header") -Header
            Write-LogMessage("[$($ScriptName), Ln: $(Get-LineNum)] $Header")
            $Data = Get-ItemProperty "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Map Network Drive MRU" |
...
function Get-IeExtensions {
    ...
    $File = Join-Path -Path $SystemFolder -ChildPath "$($Num)_IeExtensions.txt"
    $FunctionName = $MyInvocation.MyCommand.Name
    $Header = "$Num Running `"$FunctionName`" function"
    try {
        # Run the commands
...
function Get-RecentAddedExeFiles {
    ...
    try {
        ...
        $ExecutionTime = Measure-Command {
            ...
            Select-Object -First 50 | Format-Table PSPath, LastWriteTime
            if ($Data.Count -eq 0) {
                Write-NoDataFound $FunctionName
            }
            else {
